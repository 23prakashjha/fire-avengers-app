...
function UserLogin({ onLogin, addToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.user.role === 'user') {
        ...
        addToast({ type: 'success', title: 'Welcome back!', message: `Signed in as ${response.data.user.username}` });
        navigate('/dashboard');
      } else {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center user-gradient py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/4 w-48 h-48 bg-fuchsia-400/10 rounded-full blur-3xl animate-float" style={{ animationDelay: '2.5s' }}></div>
      </div>

...
function AdminRegister({ onLogin, addToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      addToast({ type: 'success', title: 'Admin Account Created!', message: 'Full system access granted' });
      ...
      <div className="max-w-md w-full relative animate-scale-in">
        <div className="rounded-2xl p-[1.5px] bg-gradient-to-br from-orange-400/60 via-white/20 to-red-500/60 shadow-dramatic">
          <div className="bg-white/95 backdrop-blur-xl rounded-2xl p-8 sm:p-10 shadow-none border-0 relative overflow-hidden">
            ...
            <div className="relative text-center mb-8">
              ...
              <p className="mt-3 text-gray-500 text-sm">Full system access for administrators</p>
            </div>
            ...
            <form className="relative space-y-5" onSubmit={handleSubmit}>
              ...
              <div className="space-y-4">
                <div className="relative group">
                  ...
                  <input name="username" type="text" required className="input-field pl-11 rounded-xl" placeholder="Choose admin username" value={formData.username} onChange={handleChange} autoFocus />
                </div>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <svg className="h-5 w-5 text-fire-400 group-focus-within:text-fire-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <input name="email" type="email" required className="input-field pl-11 rounded-xl" placeholder="Enter admin email" value={formData.email} onChange={handleChange} />
                </div>
                <div className="relative group">
                  <div className="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
                    <svg className="h-5 w-5 text-fire-400 group-focus-within:text-fire-600 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <input name="password" type="password" required className="input-field pl-11 rounded-xl" placeholder="Create a strong password" value={formData.password} onChange={handleChange} />
                </div>
                {formData.password && (
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
                <Link to="/admin/login" className="text-fire-600 hover:text-fire-700 font-semibold transition-colors group">
                  Already have an admin account? <span className="underline underline-offset-2 group-hover:text-fire-800">Sign in</span>
                </Link>
        ...
        <div className="mt-6 text-center animate-fade-in" style={{ animationDelay: '0.5s' }}>
          <p className="text-white/70 text-xs font-medium flex items-center justify-center gap-1.5">
            ...
            Admin registration - Full system access
          </p>
        </div>
      </div>
    </div>
